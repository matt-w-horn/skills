...
The elements above are checked, not hoped for. In any mode - full run, light mode, or a review - the last act before handing anything over is to reread it and confirm each element is present and doing its job: a falsifier that someone could actually observe by a rough date rather than a restated risk, paths whose one-sentence bets differ, numbers that were derived rather than asserted. This check exists because writing is absorbing enough that requirements go missing while the rules demanding them sit in plain view; the writer cannot feel the omission, and only a reread as the document's harshest reader catches it. Offer conversion to PDF or docx after they have read it, not before.
