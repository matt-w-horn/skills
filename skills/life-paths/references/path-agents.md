...
Goal: several genuinely different paths, produced by parallel agents that see the same complete picture but each optimize through one lens. The lens structure exists to fight the strongest failure mode of plan-writing: producing one path in different costumes. An agent told to find "the best independent path" is unlikely to converge with an agent told to find "the best institutional path". But the lenses are scaffolding, not a guarantee; two lenses can still land on the same bet dressed differently, so whether the drafts are genuinely plural remains a judgment you make by naming each draft's underlying bet in one sentence and comparing the sentences.
...
> - Respect the hard constraints and financial dependencies in the analysis; where your path strains a soft constraint, price it explicitly. If your path changes what they earn, price the new income from the outside view - what that work typically pays at the entry point they would actually have, verified by search, not assumed - against the personal spending floor in the financial summary, and show the comparison.
