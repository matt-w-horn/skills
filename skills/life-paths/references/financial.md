...
Spending deserves special care because it is where models most often lie. Ask for actuals: card and bank exports, a year of statements, whatever exists. If only estimates are available, take them, then mark the resulting model as estimate-grade in `finances.md`. When costs are shared, do the division on the spot: a household total plus a stated split ("we split it 60/40", "I cover the mortgage, she covers the rest") determines what this person personally must produce each month, and every question about what they can afford to risk alone turns on that personal number, not the household one. Derive it, state it, and use it everywhere the household total is tempting.
